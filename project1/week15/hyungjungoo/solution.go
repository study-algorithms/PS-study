...
type Graph map[int]EdgePq

func (ep EdgePq) Len() int           { return len(ep) }
func (ep EdgePq) Less(i, j int) bool { return ep[i].Weight < ep[j].Weight }
func (ep EdgePq) Swap(i, j int)      { ep[i], ep[j] = ep[j], ep[i] }
...
func traverse(graph Graph, current, end int, visited map[int]bool, route []int, routes *[][]int, gates []int, summits []int) {
	if current == end {
		*routes = append(*routes, append([]int{}, route...))
